...
{ // custom comparator for waitingTaskQ
...
{ // custom comparator for ActiveTaskQ
	bool operator()(openTask p1, openTask p2)
	{
		// return "true" if "p1" is ordered
		// before "p2", for example:
...
SchedulerOpen::SchedulerOpen(ThreadManager *thread_manager)
	: SchedulerPinnedBase(thread_manager, SubsecondTime::NS(Sim()->getCfg()->getInt("scheduler/pinned/quantum"))), m_interleaving(Sim()->getCfg()->getInt("scheduler/pinned/interleaving")), m_next_core(0)
{
...
bool SchedulerOpen::schedule(int taskID, bool isInitialCall, SubsecondTime time)
{
	...
	else
	{
		...
		if (waitingTaskQ.size() > 1)
		{
			...
			if (tempo.taskID != waitingTaskQ.top().taskID)
			{ // making sure that one task doesn't get pushed to waitingTaskQ twice
	...
	{ // If priority queuing is adopted, then we need to check for it and readjust tasks if required
...
{
	return ((newFrequency == oldFrequency - frequencyStepSize) && (downscalingPatience.at(coreCounter) > 0)) || ((newFrequency == oldFrequency + frequencyStepSize) && (upscalingPatience.at(coreCounter) > 0));
}
...
void SchedulerOpen::periodic(SubsecondTime time)
{
	...
	{ // Error Checking at every 1ms. Can be faster but will have overhead in simulation time.
	...
	if (time.getNS() % mappingEpoch == 0)
	{
		...
		{
			if (!schedule(taskFrontOfQueue(), false, time))
				break; // Scheduler can't map the task in front of queue.
		}
